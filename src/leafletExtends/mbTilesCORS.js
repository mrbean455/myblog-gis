...
L.TileLayer.MBTilesCORS = L.TileLayer.extend({
    ...
    initialize: function (databaseUrl, options) {
        ...
        if (typeof databaseUrl === 'string') {
            console.log('fetch')
            fetch(databaseUrl, { mode: "no-cors" }).then((response) => {
                console.log(123)
...
L.tileLayer.mbTilesCORS = function (databaseUrl, options) {
    return new L.TileLayer.MBTilesCORS(databaseUrl, options);
}
